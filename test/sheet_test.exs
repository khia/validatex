defmodule Validatex.SheetTest do
  ...
  test :no_errors do
    plan = [{"age", "30", %V.Numericality{allow_string: true}}]
  ...
  test :errors do
    plan = [{"age", "30", %V.Numericality{allow_string: false}},
            {"age", 30, %V.Range{to: 18}}
           ]
  ...
  test :errors_bang do
    ...
    assert_raise Validatex.ValidationFailure, "Validation error on age, \"30\" does not pass the validation " <>
                                              "of #{inspect %V.Numericality{allow_string: false}} with a reason of :string_not_allowed", fn ->
      V.validate!(plan)
    end
    assert_raise Validatex.MultipleValidationFailures,
        "Validation error on age, \"30\" does not pass the validation " <>
        "of #{inspect %V.Numericality{allow_string: false}} with a reason of :string_not_allowed\n" <>
        "Validation error on age, 30 does not pass the validation " <>
        "of #{inspect %V.Range{to: 18}} with a reason of :greater", fn ->
